...
🔹 牛牛开/关 - 管理插件"""
    ...
    async def _dajiao(self, event: AstrMessageEvent):
        ...
        last_time = last_actions.setdefault(group_id, {}).setdefault(user_id, {}).get('dajiao', 0)
...
添加道具 @王五 夺心魔蝌蚪罐头 1"""
    ...
    async def _admin_add_length(self, event, target_id, amount):
        ...
        yield event.plain_result(f"✅ 成功给用户 {target_data['nickname']} 添加 {amount} 长度")
    ...
    async def _admin_add_hardness(self, event, target_id, amount):
        ...
        # 添加硬度
        current_hardness = target_data.get('hardness', 1)
        updated_data = {
            'hardness': max(1, current_hardness + int(amount))
        }
        self.update_user_data(group_id, target_id, updated_data)

        yield event.plain_result(f"✅ 成功给用户 {target_data['nickname']} 添加 {amount} 硬度")
    ...
    async def _admin_add_item(self, event, target_id, item_name, amount):
        ...
        # 添加道具
        current_items = target_data.get('items', {})
        current_items[item_name] = current_items.get(item_name, 0) + int(amount)
        updated_data = {
            'items': current_items
        }
        self.update_user_data(group_id, target_id, updated_data)
    ...
    async def _admin_view_user(self, event, target_id):
        ...
        # 构建信息
        info = f"👤 用户信息\n"
        info += f"昵称：{target_data.get('nickname', '未知')}\n"
        info += f"长度：{self.format_length(target_data.get('length', 0))}\n"
        info += f"硬度：{target_data.get('hardness', 1)}\n"
        info += f"金币：{target_data.get('gold', 0)}\n"
        info += f"道具：{len(target_data.get('items', {}))} 种"

        if target_data.get('items'):
            info += "\n道具列表："
            for item, count in target_data['items'].items():
                info += f"\n  - {item} x{count}"
