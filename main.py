...
class NiuniuPlugin(Star):
    ...
    def _load_niuniu_texts(self):
        ...
        default_texts = {
        'register': {
        'success': "🧧 {nickname} 成功注册牛牛！\n📏 初始长度：{length}cm\n💪 硬度等级：{hardness}", 'already_registered': "⚠️ {nickname} 你已经注册过牛牛啦！", }, 'dajiao': {
        'cooldown': [
        "⏳ {nickname} 牛牛需要休息，{remaining}分钟后可再打胶", "🛑 冷却中，{nickname} 请耐心等待 (＞﹏＜)"
        ], 'increase': [
        "🚀 {nickname} 打胶成功！长度增加 {change}cm！", "🎉 {nickname} 的牛牛茁壮成长！+{change}cm"
        ], 'decrease': [
        "😱 {nickname} 用力过猛！长度减少 {change}cm！", "⚠️ {nickname} 操作失误！-{change}cm"
        ], 'decrease_30min': [
        "😱 {nickname} 用力过猛！长度减少 {change}cm！", "⚠️ {nickname} 操作失误！-{change}cm"
        ], 'no_effect': [
        "🌀 {nickname} 的牛牛毫无变化...", "🔄 {nickname} 这次打胶没有效果"
        ], 'not_registered': "❌ {nickname} 请先注册牛牛"
        }, 'my_niuniu': {
        'info': "📊 {nickname} 的牛牛状态\n📏 长度：{length}\n💪 硬度：{hardness}\n📝 评价：{evaluation}", 'evaluation': {
        'short': ["小巧玲珑", "精致可爱"], 'medium': ["中规中矩", "潜力无限"], 'long': ["威风凛凛", "傲视群雄"], 'very_long': ["擎天巨柱", "突破天际"], 'super_long': ["超级长", "无与伦比"], 'ultra_long': ["超越极限", "无人能敌"]
        }, 'not_registered': "❌ {nickname} 请先注册牛牛"
        }, 'compare': {
        'no_target': "❌ {nickname} 请指定比划对象", 'target_not_registered': "❌ 对方尚未注册牛牛", 'cooldown': "⏳ {nickname} 请等待{remaining}分钟后再比划", 'self_compare': "❌ 不能和自己比划", 'win': [
        "🎉 {winner} 战胜了 {loser}！\n📈 增加 {gain}cm", "🏆 {winner} 的牛牛更胜一筹！+{gain}cm"
        ], 'lose': [
        "😭 {loser} 败给 {winner}\n📉 减少 {loss}cm", "💔 {loser} 的牛牛不敌对方！-{loss}cm"
        ], 'draw': "🤝 双方势均力敌！", 'double_loss': "😱 {nickname1} 和 {nickname2} 的牛牛因过于柔软发生缠绕，长度减半！", 'hardness_win': "🎉 {nickname} 因硬度优势获胜！", 'hardness_lose': "💔 {nickname} 因硬度劣势败北！", 'user_no_increase': "😅 {nickname} 的牛牛没有任何增长。"
        }, 'ranking': {
        'header': "🏅 牛牛排行榜 TOP10：\n", 'no_data': "📭 本群暂无牛牛数据", 'item': "{rank}. {name} ➜ {length}"
        }, 'menu': {
        'default': """📜 牛牛菜单：
        🔹 注册牛牛 - 初始化你的牛牛
        🔹 打胶 - 提升牛牛长度
        ...
        'enable': "✅ 牛牛插件已启用", 'disable': "❌ 牛牛插件已禁用"
        }
        }
        try:
    ...
    def update_user_data(self, group_id, user_id, updates):
        ...
        'nickname': '', 'length': 0, 'hardness': 1, 'coins': 0, 'items': {}
        })
                                ...
                                return
                                    ...
                                    async for result in self._show_admin_menu(event):
                                        ...
                                        if msg.startswith("添加金币"):
                                            ...
                                            parts = msg.split()
                                            if len(parts) >= 3:
                                                target_id = self.parse_target(event) # 解析@的用户
                                                if target_id:
                                                    amount = parts[2]
                                                                                                                                                                        ...
                                                                                                                                                                        if is_rushing:
                                                                                                                                                                            ...
                                                                                                                                                                            async for result in self.games.fly_plane(event):
                                                                                                                                                                                yield result
                                                                                                                                                                                    ...
                                                                                                                                                                                    # 处理其他命令
                                                                                                                                                                                    handler_map = {
                                                                                                                                                                                    "注册牛牛": self._register, "打胶": self._dajiao, "我的牛牛": self._show_status, "比划比划": self._compare, "牛牛排行": self._show_ranking, "牛牛商城": self.shop.show_shop, "牛牛购买": self.shop.handle_buy, "牛牛背包": self.shop.show_items
                                                                                                                                                                                    }
    ...
    async def on_private_message(self, event: AstrMessageEvent):
        ...
        msg = event.message_str.strip()
        niuniu_commands = [
        "牛牛菜单", "牛牛开", "牛牛关", "注册牛牛", "打胶", "我的牛牛", "比划比划", "牛牛排行", "牛牛商城", "牛牛购买", "牛牛背包", "开冲", "停止开冲", "飞飞机"
        ]
    ...
    async def _toggle_plugin(self, event, enable):
        ...
        if not self.is_admin(user_id):
            ...
            return
            self.update_group_data(group_id, {
    ...
    async def _register(self, event):
        ...
        if not group_data.get('plugin_enabled', False):
            ...
            if self.get_user_data(group_id, user_id):
                ...
                user_data = {
                'nickname': nickname, 'length': random.randint(cfg.get('min_length', 3), cfg.get('max_length', 10)), 'hardness': 1, 'coins': 0, 'items': {}
                }
                                ...
                                if current_time - last_compare_time > 600:
                                    ...
                                    if compare_count >= 3:
                                        ...
                                        if user_items.get("夺心魔蝌蚪罐头", 0) > 0:
                                            ...
                                            if is_admin_user:
                                                # 管理员逻辑：50%夺取对方长度，50%无效（移除清空自己的惩罚）
                                                if effect_chance < 0.5: # 50%的概率夺取对方全部长度
                                                    ...
                                                    return
                                                        ...
                                                        # 普通用户逻辑：保持原样（50%夺取，10%清空自己，40%无效）
                                                        if effect_chance < 0.5: # 50%的概率夺取对方全部长度
                                                            original_target_length = target_data['length']
                                                            updated_user = {
                                                            'length': user_data['length'] + original_target_length
                                                            ...
                                                            if random.random() < win_prob:
                                                                ...
                                                                nickname = nickname, target_nickname = target_data['nickname'], gain = gain
                                                                )
                                                                total_gain = gain
                            ...
                            evaluation = random.choice(self.niuniu_texts['my_niuniu']['evaluation']['long'])
                                ...
                                evaluation = random.choice(self.niuniu_texts['my_niuniu']['evaluation']['very_long'])
                                    ...
                                    evaluation = random.choice(self.niuniu_texts['my_niuniu']['evaluation']['super_long'])
                                        ...
                                        nickname = nickname, length = length_str, evaluation = evaluation
                                        )
                                        yield event.plain_result(text)
            ...
            if isinstance(data, dict) and 'length' in data
                ...
                if not valid_users:
                    ...
                    for idx, (uid, data) in enumerate(sorted_users, 1):
                        ...
                        rank = idx, name = data['nickname'], length = self.format_length(data['length'])
                        )
                        )
                        yield event.plain_result("\n".join(ranking))
    ...
    async def _admin_reset_user(self, event, target_id):
        ...
        if not self.is_admin(user_id):
            ...
            if not target_data:
                ...
                # 重置用户数据
                reset_data = {
                'length': 10, 'hardness': 10, 'gold': 0, 'nickname': target_data['nickname']
                }
